...
UserSchema.methods.generateAuthToken = function(){
    ...
    return user.save().then((savedUserData) => {
        ...
        return Promise.reject("Unable to store userAuthToken");
    });
};
...
UserSchema.statics.findByToken = function(token){
    ...
    try {
        ...
        return Promise.reject("Error: Token unhashing issue");
    }
...
UserSchema.statics.getItemsByUserId = function(id) {
    ...
    return useritems = UserTransaction.find({
        ...
        if(!usertransactions) {
            return Promise.reject("Usertransaction not found");
        }
...
UserSchema.statics.initConfigSetting = function() {
    // var Configsetting = new Configsetting();
    if(Object.keys(config).length > 0) {
        Configsetting.remove({}).then(() => {
            var setting_data = [];
            Object.keys(config).forEach(function(key,index) {
                setting_data.push({
                    'setting_name':key,
                    'setting_value':JSON.stringify(config[key]),
                    'created_by':'system',
                    'createdAt':moment().valueOf(),
                });
            });
            return Configsetting.insertMany(setting_data);
        }).then((configsettingdata) => {
            // console.log(configsettingdata);
            return new Promise((resolve, reject) => {
                resolve(configsettingdata);
            });
        }).catch((err) => {
            console.log(err);
        });
